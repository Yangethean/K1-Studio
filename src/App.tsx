...
export default function App() {
  ...
  const handleGenerateSingle = async (e: React.MouseEvent, sub: Subtitle) => {
    ...
    try {
      blob = await generateSubtitleAudio(sub);
      if (blob) {
        url = createTrackedUrl(blob);
        try {
          duration = await getAudioDuration(url);
        } catch (durErr) {
          console.warn("Could not determine audio duration, using 1.5s fallback:", durErr);
          duration = 1.5;
        }
        try {
          waveformPeaks = await generateWaveform(url);
        } catch (waveErr) {
          console.warn("Waveform generation failed, continuing with empty peaks:", waveErr);
          waveformPeaks = [];
        }
      }
    } catch (err: any) {
      console.error("Failed to generate audio clip:", err);
    ...
    updateSubtitles((prev) =>
      prev.map((s) => (s.id === sub.id ? {
        ...s,
        isGenerating: false,
        audioUrl: url || s.audioUrl || undefined,
        audioBlob: blob || s.audioBlob || undefined,
        audioDuration: (duration > 0 ? duration : s.audioDuration) || undefined,
        waveformPeaks: waveformPeaks || s.waveformPeaks,
        audioBufferId: url || s.audioBufferId || undefined,
        audioTrimStart: 0,
        audioTrimEnd: (duration > 0 ? duration : s.audioTrimEnd) || undefined,
        audioStartTime: s.audioStartTime ?? suggestedStart
      } : s))
    );
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < subtitles.length; i++) {
        ...
        try {
          blob = await generateSubtitleAudio(subtitles[i]);
          if (blob) {
            url = createTrackedUrl(blob);
            try {
              duration = await getAudioDuration(url);
          ...
          console.error(`Failed to generate audio for subtitle index ${i}:`, err);
          setErrorMsg(`Failed to generate clip: ${err.message || String(err)}`);
        }
